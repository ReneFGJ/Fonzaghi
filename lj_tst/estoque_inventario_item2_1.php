...
if (pg_num_rows($rlt) > 0){
	echo '<form action="estoque_inventario_item2_1.php?data='.$data.'&codigo='.$cod_produto.'&status='.$status.'" method="post">';
	echo '<table border="0"  class="1_naoLinhaVertical" width="'.$tab_max.'" align="center" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF">';
	echo '<tr><td colspan="5" align="center" class="lt5">'.$titulo.'</td></tr>';
	echo '<tr>';
	echo '<th class="1_th">Produto</th>';
	echo '<th class="1_th">EAN13</th>';
	echo '<th class="1_th">Comentário</th>';
	...
	for($i=0; $i < pg_num_rows($rlt); $i++){
		...
		echo '<td class="1_td" align="center"><input type="text" name="txt_comentario[]" size="20" value="'.trim($line['i_comentario_supervisor']).'" maxlength="50"></td>';
		if (strlen(trim($line['i_comentario_diretoria'])) > 0)
			echo '<td class="1_td" align="center">'.$line['i_comentario_diretoria'].'</td>';
		else
	...
	echo '<tr><td colspan="4" class="rodapetotal">'.$itens.' ítnes</td></tr>';
	echo '<tr><td colspan="4" align="center"><input type="submit" name="bt_gravar" value="Gravar">&nbsp;&nbsp;';
...
function acao(){
	...
	if (strlen($_POST['bt_gravar']) > 0){
		for($i=0; $i<count($_POST['txt_ean13']); $i++){
			if (strlen($_POST['txt_comentario'][$i]) > 0){
				$sql="UPDATE inventario
   						SET i_supervisor='".$user_log."', i_comentario_supervisor='".$_POST['txt_comentario'][$i]."'
						WHERE i_ean13='".$_POST['txt_ean13'][$i]."'";

				//echo '<br>sql='.$sql;
				$rlt=db_query($sql);
			}
		}
		redirecina("estoque_inventario_item2.php");
	}
