...
echo '<h1>Relatório produtos mais vendidos</h1>';
$tela = $form->editar($cp,'');

if ($form->saved > 0)
	{
	$sql = "select p_codigo, p_descricao, count(*) as pecas_vendidas, ";
	$sql .= " sum(pe_vlr_vendido) as vlr_venda, sum(pe_vlr_custo) as vlr_custo, sum(pe_vlr_venda-pe_vlr_custo) as vlr_lucro ";
	...
	$sql .= " group by p_codigo, p_descricao ";
	$sql .= " order by pecas_vendidas desc ";
	
	$sql_totais = "select count(*) as pecas_vendidas, ";
	$sql_totais .= "		sum(pe_vlr_vendido) as vlr_venda,  ";
