...
array_push($breadcrumbs, array('produtos_rastreio.php','Rastreio de produto'));
...
<?
if (strlen($dd[1]) == 0) { $dd[1] = date("01/01/Y"); }
if (strlen($dd[2]) == 0) { $dd[2] = date("d/m/Y"); }
$cp = array();
array_push($cp,array('$S12','','Código do Produto ',True,True,''));
array_push($cp,array('$D8','','De ',True,True,''));
array_push($cp,array('$D8','','Até ',True,True,''));

echo '<CENTER><h1>Relatório de Rastreio de Peças</h1>';
...
(strlen($dd[0]) <= 6)?$campo='pl_produto':$campo='pl_ean13';
...
if (strlen($dd[0]) == 12)
	{
	$sql = "select * from produto_estoque where pe_ean13 = '".$dd[0]."' ";
	$rlt = db_query($sql);
	$line = db_read($rlt);
...
echo '<center><h2>De ' .$dd[1]. ' até '.$dd[2].'</h2></center>';
echo '<br>';
