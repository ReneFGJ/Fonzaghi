		...
		function caixa_notas_abertas($lote='')
			{
				...
				while ($line = db_read($rlt))
				{
					...
					if (($tipo=='NOA') or ($tipo =='NOT'))
						{
							if ($cab==1) { $sn .= '<TR><TD colspan=6 class="lt3"><B>Notas Promissórias</b></td>'; }
							$sn .= '<TR '.coluna().'>';
							$sn .= '<TD>';
							$sn .= $line['cx_nome'];
					...
					if (($tipo=='VIS') or ($tipo =='MAS') or ($tipo =='RED') or ($tipo =='ELC') or ($tipo =='HIP'))
						{
							if ($cab==1) { $sc .= '<TR><TD colspan=6 class="lt3"><B>Cartao crédito/débito - '.$tipo.'</b></td>'; }
							$sc .= '<TR '.coluna().'>';
							$sc .= '<TD>';
							$sc .= $line['cx_nome'];
		...
		function caixa_resumo($lote='')
			{
				...
				$sx .= '<TR><TH>Descrição<TH width="120">Créditos<TH width="120">Débitos';
				$rlt = db_query($sql);	
				$sld1 = 0;
				$sld2 = 0;
				$din = 0;
				...
				while ($line = db_read($rlt))
					{
						...
						if ($line['cx_tipo'] <> 'MAN')
							...
							if ($tipo == 'NOA') { $tipo = 'Nota Promissória emitidas'; $col=0; $fat = $fat + $vlr; }
							if ($tipo == 'NOT') { $tipo = 'Abertura de Nota Promissória (old)'; $col=0;  $fat = $fat + $vlr;}
							...
							if ($tipo == 'CRE') { $tipo = 'Crédito de cliente'; $col=1; $fat = $fat + $vlr;}
							if ($tipo == 'CRD') { $tipo = 'Crédito para cliente'; $col=0;  $fat = $fat + $vlr; }
							if ($tipo == 'TRO') { $tipo = 'Devolução de troco em dinheiro'; $col=1; $din = $din + $vlr;  $fat = $fat + $vlr; }
							if ($tipo == 'SLD') { $tipo = 'Fechamento de Caixa'; $col=1;   }
							if ($tipo == 'DEP') { $tipo = 'Depósito no Banco'; $col=0;   }
		...
		function cx_open_cp()
			{
				...
				//array_push($cp,array('$Q ac_descricao:ac_codigo:select * from area_conhecimento where ac_area_root = 1 ','ac_codigo_mae','Área do conhecimento',False,True,''));
		...
		function rever_transacoes()
			{
				...
				$sx .= '<h2>Transações abertas (sem fechamento de lote)</h2>';
				...
				$sta = array('@'=>'Em transação','A'=>'Finalizada');
				$rlt = db_query($sql);
				while ($line = db_read($rlt))
					{
						$link = '';	
		...
		function mostra_lancamentos($drp)
			{
				...
				$sx .= utf8_encode('<TR><TH>Descrição<TH>Cliente<TH>Tipo<TH>Débito<TH>Crédito');
		...
		function caixa_saldo_notas($lj,$id,$check,$cliente)
			{
				...
				if (strlen($dp)==0)
					{
						echo 'Nome da Loja nao está definido';
						exit;
					}
				if (strlen($id) > 0)
					{
		...
		function caixa_recibos()
			{
				...
				$sx .= '<TR><TH>Nome<TH>Histórico<TH>Valor<TH>Juros<TH>Dt.Liq.';
				while ($line = db_read($rlt))
					{
		...
		function cp_FUN()
			{
				...
				array_push($cp,array('$H8','','',False,True));
				array_push($cp,array('$U8','','',False,True));
				array_push($cp,array('$N8','',utf8_encode('Valor para débito'),True,True));
				return($cp);
			}			
		function cp_TRO()
			{
		...
		function cp_CRE()
			{
				...
				array_push($cp,array('$H8','','',False,True));
				array_push($cp,array('$U8','','',False,True));
				array_push($cp,array('$N8','','Valor para enviar para Crédito',True,True));
				return($cp);
			}
		function cp_CRD()
			{
		...
		function cp_CHQ()
			{
						...
						if ($cmc->valid($cmc7)==1)
							{
								...
								$msg = '<TD><font color="red"><B>ERRO NOS DÍGITOS DO CHEQUE</font>';
								$dd[7] = '';
								$dd[8] = '';
								$dd[9] = '';
								$dd[10] = '';
				...
				array_push($cp,array('$D8','',utf8_encode('Pré-datado'),True,True));
				array_push($cp,array('$S4','','Banco',True,False));
				array_push($cp,array('$S6','',utf8_encode('Agência'),True,False));
		...
		function cp_DEP()
			{
				...
				array_push($cp,array('$H8','','',False,True));
				array_push($cp,array('$H8','','',False,True));
				array_push($cp,array('$N8','',utf8_encode('Valor do Depósito'),True,True));
				array_push($cp,array('$D8','',utf8_encode('Data depósito'),True,True));
		...
		function cp_ABO()
			{
				...
				array_push($cp,array('$S10','',utf8_encode('Nº anomalia'),True,True));
				array_push($cp,array('$H8','','',False,True));
				return($cp);
			}				
		function cp_MAS()
			{
				...
				array_push($cp,array('$D8','',utf8_encode('Data lançamento'),True,True));
				array_push($cp,array('$A','',utf8_encode('MASTERCARD - Crédito'),False,True));
		...
		function cp_VIS()
			{
				...
				array_push($cp,array('$N8','','Valor do Ticket',True,True));
				array_push($cp,array('$D8','',utf8_encode('Data lançamento'),True,True));
				array_push($cp,array('$A','',utf8_encode('Visa - Crédito'),False,True));
		...
		function cp_ELC()
			{
				...
				array_push($cp,array('$N8','','Valor do Ticket',True,True));
				array_push($cp,array('$D8','',utf8_encode('Data lançamento'),True,True));
				array_push($cp,array('$A','',utf8_encode('Visa Electron - Débito'),False,True));
				array_push($cp,array('$H4','','',False,False));
		...
		function cp_RED()
			{
				...
				array_push($cp,array('$N8','','Valor do Ticket',True,True));
				array_push($cp,array('$D8','',utf8_encode('Data lançamento'),True,True));
				array_push($cp,array('$A','',utf8_encode('RedeCard - Débito'),False,True));
				array_push($cp,array('$H4','','',False,False));
				array_push($cp,array('$[1-1]','','Parcelas',False,False));
				array_push($cp,array('$S10','','AUT:',True,True));
				array_push($cp,array('$S6','','DOC:',True,True));
		...
		function cp_HIP()
			{
				...
				array_push($cp,array('$N8','','Valor do Ticket',True,True));
				array_push($cp,array('$D8','',utf8_encode('Data lançamento'),True,True));
				array_push($cp,array('$A','',utf8_encode('HiperCard - Débito'),False,True));
				array_push($cp,array('$H4','','',False,False));
				array_push($cp,array('$[1-12]','','Parcelas',False,False));
				array_push($cp,array('$S10','','AUT:',True,True));
				array_push($cp,array('$S6','','DOC:',True,True));
		...
		function cp_JUR()
			{
				...
				array_push($cp,array('$U8','',utf8_encode('Data lançamento'),True,True));
				array_push($cp,array('$H8','','',False,True));
				array_push($cp,array('$H4','','',False,False));
				array_push($cp,array('$H4','','',False,False));
				array_push($cp,array('$H4','','',False,True));
				...
				if ($tipo == 'CRD')
					{
						...
						$sql = "insert into credito_outros 
								...
								'$loja','',$valor,
								0,'','',
								1,'$doc_nr','Uso de Crédito de Cliente'
								)
						";
						$rlt = db_query($sql);
					}
	...
	function mostra_pagamentos($acao=1)
		{
			...
			if (strlen($sx) > 0)
				{
					$sx = '<TR><TH>Valor<TH>Descrição<TH>Ação'.$sx;
				}
			return($sx);
	...
	function cx_mostra_consultoras($rlt,$page='')
		{
			...
			$sx .= '<TR class="tabela01h"><TH  >Nome da Consultora<TH>Código<TH>Dt.Nasc.<TH>Tipo';
			while ($line = db_read($rlt))
				{
