...
class pedido
	{
		...
		function row()
		{
			...
			$cdm = array('ID','Pedido','Fornecedor','Itens','Valor','Data','Status');
			$masc = array('','#','','#','$R','#','#','','');
			return(True);
		}
		...
		function pagamentos()
			{
				...
				$sql = "select * from contas_pagar 
						where cr_pedido = '$pedido' and cr_status <> 'X'
						order by cr_venc
					";
				$rlt = db_query($sql);
		...
		function uso_interno($id_ped='')
			{
            	...
            	$link_conf = '<A HREF="javascript:newxy(\'../ed_edit_pop.php?dd0='.$id_ped.'&dd99=pedido_fx\',600,600);">';
		...
		function mostra_empresa()
			{
				...
				$sx .= '<BR><NOBR><B>Atualizado :'.$hd->mostra_nome_login(trim($line['ped_login'])).'</B>';
				$sx .= '<BR><NOBR><B>Criado:'.$hd->mostra_nome_login(trim($line['ped_login_criado'])).'</B>';
				$sx .= '</table>';
				return($sx);
			}
		...
		function mostra_empresa_romaneio()
			{
				...
				$sx .= '<center><h1>Romaneio</h1></center>';
    ...
    function lista_pedidos()
        ...
        function lista_pedidos_fornecedor2($fo_codigo)
        {
        	$sx ='<TABLE width="100%" cellpadding="0" cellspacing="0" class="lt1">
					<TR>
						<TH>Ped.</TH>
						<TH>Data</TH>
						<TH>Ítems</TH>
					...
					';
			$sql = "select * from pedido 
					where ped_fornecedor = '".$fo_codigo."' 
					order by ped_nrped desc 
					limit 20 ";
			$rlt = db_query($sql);
			$temp = 0;
			while ($line = db_read($rlt))
			{
				$link = '<A HREF="javascript:newxy2(\'pedido_mostra.php?dd1='.$line['ped_nrped'].chr(39).',800,800);">';
				if ($line['ped_status']=='T') 
					{ 
					$temp++; 
					$link = '<A HREF="ped_novo_ed.php?dd0='.$line['ped_fornecedor'].'">';
					}
				$sx .= '<TR '.coluna().'>';
				$sx .= '<TD align="center">'.$link.$line['ped_nrped'].'</TD>';
				$sx .= '<TD align="center">'.$link.$line['ped_data'].'</TD>';
				$sx .= '<TD align="center">'.$link.$line['ped_items'].'</TD>';
				$sx .= '<TD align="right">'.$link.number_format($line['ped_valortotal'],2).'</TD>';
				$sx .= '<TD align="center">'.$link.$line['ped_status'].'</TD>';
				$sx .= '</TR>';
			}
			$sx .= '</TR></TABLE>';
		    return($sx);
        ...
        function botao_novo_pedido($cod_forn)
        {
		...
		if ($temp == 0)
			{
			$sx .= '<BR><CENTER>';
			$sx .= '<form method="post" action="ped_novo_ed.php?dd0='.$cod_forn.'">';
			$sx .= '<input type="submit" name="acao" value="'.$bb1.'" style="width:300; height:40;">';
			$sx .= '</form>';
		     
		} else {
			$sx .= 'Existe '.$temp.' pedidos temporários deste cliente, não é possível abrir um novo';
		}
        	return($sx);
        }
        
        function busca_pedido_aberto($cod_forn)
        {
        	$sql = "select * from pedido ";
			$sql .= " where ped_fornecedor = '".$cod_forn."' and ped_status = 'T' ";
			$rlt = db_query($sql);
		...
		function cp_item()
		{
			...
			$cp = array();
			array_push($cp,array('$H8','id_pedi','',False,True,''));
			array_push($cp,array('$S10','pedi_ref_forn','Ref Forn.',True,True,''));
			array_push($cp,array('$S10','pedi_ref_item','Ref Fonz.',True,True,''));
			array_push($cp,array('$S60','pedi_descricao','Descrição.',True,True,''));
			array_push($cp,array('$S6','pedi_quan','Quantidade',True,True,''));
			array_push($cp,array('$S6','pedi_vlrunit','Valor Unit.',True,True,''));
			array_push($cp,array('$S6','','Valor Repas.',True,True,''));
			array_push($cp,array('$S6','','Valor Final.',True,True,''));
			array_push($cp,array('$S10','','Valor Total.',True,True,''));
			
			return($cp);
		}
		
		function menu($cod_forn)
		{
			$sx = '<table cellpadding="0" cellspacing="2" width="98%" class="lt1">
					<TR align="center" bgcolor="#c0c0c0">
						<TD><A HREF="ped_novo_ed.php?dd0='.$cod_forn.'&dd90='.$dd[90].'&dd50=0"><font size="4" color="#FFFFFF">Dados de Faturamento</A></TD>
						<TD><A HREF="ped_novo_ed.php?dd0='.$cod_forn.'&dd90='.$dd[90].'&dd50=1"><font size="4" color="#FFFFFF">Fornecedor</A></TD>
						<TD><A HREF="ped_novo_ed.php?dd0='.$cod_forn.'&dd90='.$dd[90].'&dd50=2"><font size="4" color="#FFFFFF">Itens</A></TD>
						<TD><A HREF="ped_novo_ed.php?dd0='.$cod_forn.'&dd90='.$dd[90].'&dd50=3"><font size="4" color="#FFFFFF">Dados Gerais</A></TD>
						<TD><A HREF="ped_novo_ed.php?dd0='.$cod_forn.'&dd90='.$dd[90].'&dd50=4"><font size="4" color="#FFFFFF">Uso Interno</A></TD>
						<TD><A HREF="ped_novo_ed.php?dd0='.$cod_forn.'&dd90='.$dd[90].'&dd50=5"><font size="4" color="#FFFFFF">Programação e Pagamentos</A></TD>
					</TR>
					</table>';
			return($sx);
		}
