...
class cep
	{
		...
		/*Não finalizado - falta incrementar os unions entre os selects*/
		/*gera uma query de tabela temporaria para ser usada em bases diferentes
		function query(){
			$wh = '';
			if(strlen($this->uf)>0){
				$wh = " where tb.uf='".$this->uf."' ";
			}					
			$sql = 'select cep, uf, bairron, nbairro, logradouro from bairro 
					inner join (select * from cep) as tb 
					on tb.bair_dne=bairro.key_dne
					'.$wh;
			$rlt = db_query($sql);
			
			while($line = db_read($rlt)){
				$sx .= "select ".$line['cep']." as cep, 
								".$line['uf']." uf, 
								".$line['bairron']." bairro,
								".$line['logradouro']." rua
								 ";	
