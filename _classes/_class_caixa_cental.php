...
class caixa_central
	...
	function saldo_receita($d1,$d2,$nohd=0)
		{
			$sql = "";
			for ($r=1;$r <=6;$r++)
				{
			...
			$sx .= '<TR>
					<TD>'.substr($d1,4,2).'/'.substr($d1,0,4).'
					...
					<TD class="tabela01" align="right"><B>'.fmt($out+$bco+$car+$din,2).'</B>';
			if ($nohd==2)
				{
				$sx .= '</table>';
				}
			...
			while ($line = db_read($rlt))
				{
					...
					$link_pedido_a = '</A>';
					$link_pedido = $this->link_pedido($pedido);
