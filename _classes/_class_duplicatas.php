 ...
 */
	...
	var $id_dp; 
  	...
  	var $dp_nrop;
/*	
	function set_duplicatas($cliente='',$tb){
		global $base_name,$base_host,$base_user;	
		require("../db_fghi_210.php");	
		$this->set_tabelas($tb);
				
		$sql = 'select * from '.$tb.' where dp_cliente = '.$cliente;
...
*/			
    ...
    function razao_arquivo_morto($dt1,$dt2,$lj){
        $tx=''; 
        ...
        $sql = "select * from duplicata_arq_morto where $tx order by dp_venc";
        ...
        while ($line=db_read($rlt)) {
            $sx .= '<tr class="lt1">';
            $sx .= '<td>'.$line['dp_cliente'].'</td>';
            $sx .= "<td>".$line['dp_valor']."</td>";
            $sx .= "<td>".$line['dp_venc']."</td>";
            $sx .= "<td>".$line['dp_loja']."</td>";
            $sx .= "</TR>";
        }
        $sx .= "</font></table>";
    return($sx);
    ...
    function transf_arquivo_intermediario_morto(){
            ...
            $mes = date('m');    
            $dia = date('d');    
            $dt  = date("Ydm", mktime(0, 0, 0, ($mes-6), $dia ,($ano-(4)))); 
            $this->set_tabelas();
            $tot=0;
            ...
            for ($i=0; $i < count($this->dupl); $i++) 
                {
                       ...
                       $rlt2 = db_query($sql2);
                       echo '<BR>--'.$lj.'-'.$tb;
                       echo "---". $dt  = date("Ydm", mktime(0, 0, 0, ($mes-6), $dia ,($ano-(4)))); 
