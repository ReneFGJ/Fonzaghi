...
class funcionario
	...
	function mostra()
		...
		$dir_img = '/dados/imagens/foto/'.$img;
		...
		$img = '<IMG SRC="'.$http_public.'/funcionario/foto/'.trim($img).'" width="100" border="1"></A>';
	...
	function cp()
		{
			$cp = array();
			//$grau = "1 GRAU:1ï¿½ GRAU&2 GRAU:2 GRAU&3ï¿½GRAU COM:3ï¿½GRAU COM";
			...
			//array_push($cp,array('$O '.$grau,'us_instrucao','Grï¿½u de instrucao',False,True,''));
			array_push($cp,array('$S20','us_instrucao','Grau de instrução',False,True,''));
			...
			array_push($cp,array('$[0-9]','us_nivel','Nível',False,True,''));
	...
	function perfil_funcionario(){
		global $dd;
		if (trim($dd[1])==04) {} else {$tx .= ' and us_loja like \'%'.$dd[1].'%\'';}
		if (strlen(trim($dd[2]))==0) {} else {$tx .= ' and us_departamento like \'%'.$dd[2].'%\'';}
		if ((strlen(trim($dd[3]))==0)||(round($dd[3])=='36')) {} else {$tx .= ' and us_cargo_avaliacao like \'%'.$dd[3].'%\'';}
		
		
		?>
		<FONT FACE="Tahoma, Geneva, sans-serif" size=2 color="#666666">
		Perfil dos Funcionarios<br>
		<FONT FACE="Tahoma, Geneva, sans-serif" size=7 	 color=#000000>
		<?=nomemes($dd[1])?>
		...
		$sql .= " us_status ='A' ".$tx;
		$sql .= " order by us_dtadm ";
		$pra = db_query($sql);
		
		$ss='';
		$ss=$ss.'<TABLE width="100%" class="lt1" cellpadding=0 cellspacing=0>';
		$ss=$ss.'<TR>';
		$ss=$ss.'<TD colspan="2" width="33%"><img src="../images/nadap.gif" width=1 height=1 border=0></TD>';
		$ss=$ss.'<TD colspan="2" width="33%"><img src="../images/nadap.gif" width=1 height=1 border=0></TD>';
		$ss=$ss.'<TD colspan="2" width="33%"><img src="../images/nadap.gif" width=1 height=1 border=0></TD>';
		$ss=$ss.'</TR>';
		$col=0;
		while ($line = db_read($pra))
			{
				...
				$img = '/dados/imagens/foto/'.$line["us_cracha"].'A.JPG';
				if (file_exists($img))
					{
					$img = '/fonzaghi/funcionario/foto/'.$line["us_cracha"].'A.JPG';
					} else {
					$img = '/fonzaghi/funcionario/foto/NOIMAGE.JPG';
					}
				$col = $col + 1;
				$ss.='<TD width="50"><img src="'.$img.'" width=100 border=0></TD>';
				$ss.="<TD>";
				$ss.='<FONT FACE="Tahoma, Geneva, sans-serif" size=5 color=#808080>';
	...
	function adiciona_foto($id)
	{
		$sx='<form action="upload_file.php&dd0='.$id.'" method="post"
